...
def Cliente():
    ...
    if (cedulaIngresada == IdCliente):
        ...
        Restaurant()#Llamado o invocacion a la clase MenuPedidos
    else:
        ...
        elementos de una lista
            """
        print(list)
    Restaurant()#Llamado a la clase MenuPedidos
...
"""
Esta parte nos ayuda a la impresion de menu de 
restaurantes con su comida y su precio
"""
def imprimir_menu(lista_Restaurant):
    if lista_Restaurant:
        """
        El metodo format permitira agregar en la cadena una serie de llaves y luego indicara python
        lo que se va agregar en esas llaves...
        Se puede indicar dentro de cada uno de los conectores de argunmentos {} distintas opciones
        de formato en este caso se reserva doce espacios para cada uno de los elementos. 
        """
...
def Restaurant():
    ...
    while opRestaurant != "salir":
        opRestaurant=input("Elija una opcion: listar, buscar o agregar : ")
        ...
        if opRestaurant == "listar":
            ...
            while opBuscar!="salir":
                ...
                opBuscar=opBuscar.lower() #convierte la cadena de caracteres en minusculas
                if opBuscar!= "salir":
                    lista_Restaurant=Leer_restaurant() #Permite la busca de loa argumento de la funcion Leer_restaurant
                    encontrado=[ ]
                    for item in lista_Restaurant:
                        if opBuscar in item:
...
def MenuPedidos():
        ...
        restaurante=input("Ingrese nombre del restaurant: ")
        comida=input("Ingrese el nombre del palto de comida: ")
        precio=input("Ingrese el precio de la comida: ")
        nueva_lista=str(restaurante.lower() + "-" + comida.lower() + "-" + precio)
        archivo=open("restaurant.txt",'a')
        archivo.write(nueva_lista)
        archivo.close()
        print("Se ha agrega una nueva comida al menu Nombre de restaurant: " + restaurante + " comida " + comida + " precio " + precio)
        PagoPedidos()


def PagoPedidos():
        cantidad=int(input("Ingrese cantidad de productos desea comprar: "))
        contador=0
        if(contador<=cantidad):
            precio=float(input("El precio a pagar: "))
            totalPagar=1*precio
